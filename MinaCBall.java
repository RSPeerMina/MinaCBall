...
public class MinaCBall extends Script {
    ...
    public int loop() {
        ...
        if (!Movement.isRunEnabled() && Movement.getRunEnergy() > 20 && !!WORKING && !Bank.isOpen()) {
        ...
        if (Inventory.contains("Ammo mould") && HAVE_BARS && !Local.isAnimating() && !WORKING) {
            Log.info("Smelting");
        ...
        while (Production.isOpen()) {
            ...
            Production.initiate();
            WORKING = true;
            Time.sleepUntil(() -> HAVE_BARS, 150000);
        }
