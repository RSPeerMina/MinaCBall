...
public class MinaCBall extends Script {
    ...
    public int loop() {
        ...
        if (Dialog.isViewingChat() || Bank.isOpen()) {
            WORKING = false;
        }
        ...
        if (Inventory.contains("Ammo mould") && HAVE_BARS && !Production.isOpen() && !WORKING) {
            Log.info("Smelting");
        ...
        if (Bank.isOpen() && !Bank.contains("Steel bar") && !HAVE_BARS) {
